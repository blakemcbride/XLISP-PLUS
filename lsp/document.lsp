...
;;; TODO - Added documentation for defclass and defmethod. For defmethod we need to add another
;;;    property which is an association list of methods and docs for the class. 
...
;;; DEFCONSTANT, DEFPARAMETER, DEFVAR, DEFUN, DEFMACRO, DEFSTRUCT
;;; are modified to have the documentation arguments functional. DEFCLASS has it built-in.  Note that
;;; %doc-variable is used for DEFCONSTANT, DEFPARAMETER,  DEFVAR and DEFCLASS
;;; %doc-function is used for DEFUN and DEFMACRO
;;; %doc-setf is used for DEFSETF (not implemented either in defsetf or glos)
;;; %doc-structure is used for DEFSTRUCT but doesn't show in the glos command
...
(import '(xlisp::%doc-function xlisp::%doc-variable xlisp::%doc-methods))
...
;; Glos function that reads from external file. With lots of memory
;; we use this once on each symbol to bring documentation into memory
;; then we don't need the file
...
(defun glos (symbol &optional matchall
                    &aux val val2 val3 val4 val5 val6 (sym (string symbol)))
       (catch
        'getoutahere
        (setq *linecount* 0)
        (if (and (null (eq matchall t)) (setq val (find-symbol sym)))
            (progn (if matchall ; class and selector
		       (do ()
			   ((or val2 val6) t)
			   (setq val2 (assoc matchall (get val '%doc-methods)))
			   (if val2 ;; it is documented here
			       (ppstring (cdr val2))
			       (if (and (setq val (eval val)) ;; valid superclass found
					  (classp val)        ;; so look for doc there
					  (setq val (send val :superclass)))
				   (setq val (find-symbol (send val :pname)))
				   (setq val6 t))))
;                       (when (setq val2 (assoc matchall (get val '%doc-methods)))
;                             (ppstring (cdr val2)))
                       (progn 
                              (when (setq val2 (documentation val 'function))
                                    (ppstring val2))
                              (when (setq val3 (documentation val 'variable))
                                    (ppstring val3))
                              (when (setq val4 (documentation val 'setf))
                                    (ppstring val4))
                              (when (setq val5 (documentation val 'structure))
                                    (ppstring val5))))
                   (unless (or val2 val3 val4 val5)
                           (format t (if (and matchall (null (eq matchall t))) "No information on ~a ~a~%" "No information on ~a~%") sym matchall)))
            (progn
             (setq val
                   (do ((list (apropos-list sym) (cdr list))
                       ...
                       (when (setq val2 (documentation (car list) 'variable))
                             (when (not (member val2 result :test #'string-equal))
                                   (push val2 result)))
                       (when (setq val2 (documentation (car list) 'setf))
                             (when (not (member val2 result :test #'string-equal))
                                   (push val2 result)))
                       (when (setq val2 (documentation (car list) 'structure))
                             (when (not (member val2 result :test #'string-equal))
                                   (push val2 result)))))
...
;; In the case of defun defmacro and defvar, the documentation string is removed
;; from the definition.
...
;; Make it all internal -- takes memory, but we have lots of it now
#+:largemem
(do-external-symbols
        (x :xlisp)
        (when (and (fboundp x) (numberp (get x '%doc-function)))
              (setf (get x '%doc-function) (glosx (get x '%doc-function))))
        (when (and (specialp x) (numberp (get x '%doc-variable)))
              (setf (get x '%doc-variable) (glosx (get x '%doc-variable))))
)

#+:largemem
(setf (get 'glos '%doc-function) (glosx (get 'glos '%doc-function)))      

#+:largemem
(unintern 'glosx) ;; we don't need glosx anymore
