...
*
* File:         osxstuff.c
* Description:  OS X-Specific interfaces for XLISP
...
*   Modified for Mac OS X in 2010 by Tom Almy
********************************************************************************
*/
...
VOID ostputc(int ch)
{
       ...
       (void)write(2,buf,1); // It might be redirected -- always want stderr
#ifdef GRAPHICS
        }
#endif
...
/* get the expression to evaluate */
  ...
  rdtm = (rtm > 0) ? rtm : -rtm;
  sprintf(buf, "System CPU %.2f sec., User CPU %.2f sec., Real %.2f sec.\n",
		  (tm2.tms_stime - tm1.tms_stime)/(double)ticks_per_second(),
		  (tm2.tms_utime - tm1.tms_utime)/(double)ticks_per_second(),
