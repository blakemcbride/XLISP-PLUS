...
; do not quote their arguments and work with any arbitrary object.
...
; For instance objects, instance variables can be examined
...
(defun ins-ivar (obj name &aux (sel
#+:packages	(intern (string name) :keyword))
#-:packages      (intern (strcat ":" name)))
       (if (send obj :respondsto sel)                         
           (funcall #'send obj sel)
           "???"))
...
(defun getpname (obj &aux temp)
       (if obj
           (progn
            (setf temp (send obj :pname))
            (if temp temp "???"))
           nil))
...
(defun ins-display (list)
       ...
       (cond ((typep list '(or cons array))
             ...
             ((classp list)
              (format t "Class ~a, Superclass ~a, selectors:~%" (getpname list) (getpname (send list :superclass)))
              (do* (allselectors
                     (isfirst t nil)
                     (thisclass list (send thisclass :superclass))
                     (selectors (mapcar #'car (send thisclass :messages))
                                (when thisclass (set-difference (mapcar #'car (send thisclass :messages)) allselectors))))
                   ((null thisclass) nil)
                   (unless isfirst (when selectors (format t "(In ~a)- " (getpname thisclass))))
                   (setf allselectors (append allselectors selectors))
                   (mapc #'(lambda (n) (format t " ~s" n)) selectors)
                   (terpri))
              (terpri)
              (when (send list :cvars)
                    ...
                    (mapc #'(lambda (n) (format t " ~s ~s~%" (car n) (cdr n)))
                          (send list :cvars))))
             ((objectp list)
              (format t "A ~a, ivars:~%" (getpname (send list :class)))
              (do* (allivar
                    (isfirst t nil)
                    (thisclass (send list :class) (send thisclass :superclass))
                    (ivar (send thisclass :ivars)
                          (when thisclass (set-difference (send thisclass :ivars) allivar))))
                   ((null thisclass) nil)
                   (unless isfirst (when ivar (format t "(In ~a)- " (getpname thisclass))))
                   (setf allivar (append allivar ivar))
                   (when ivar
...
(defun ins-object-spec (list &aux name)
       ...
       (when (member (setq name (ins-protread))
                     (do* ((thisclass (send list :class) (send thisclass :superclass))
                           (vars nil vars))
                          ((null thisclass) vars)
                          (setf vars (append (send thisclass :ivars) vars))))
             name))
