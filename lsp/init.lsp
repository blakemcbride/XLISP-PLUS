; initialization file for XLISP-PLUS 3.04 or 3.05

(unless (boundp 'version) (defconstant version '(3 . 4)))

(format t "XLISP-PLUS ~d.~2,'0d contains contributed code by:
...
;; Features feature -- first thing we need to know is if we have packages
(setq *features* nil)
(when (fboundp 'export)
      (setq *features* (cons :packages *features*)))

;; Add some features for our version
(setq *features* (cons :xlisp *features*))
(setq *features* (cons :21h *features*))
(setq *features* (cons :v30 *features*))
#+:packages
(setq *features* (cons
		       (intern (format nil "V~d~2,'0d" 
				       (car version) (cdr version))
			       "KEYWORD"
			       )
		       *features*))
...
(when (fboundp 'mode)
      (setq *features* (cons :graphics *features*)))
(when (fboundp 'brush)
      (setq *features* (cons :qt *features*)))
...
(princ "\nInitialization file (init.lsp) loaded.\n")
