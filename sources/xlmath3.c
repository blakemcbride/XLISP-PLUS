        ...
        if (which != 'I' && ! zerop(r)) {
            switch (which) {
                ...
                case '_':
                    ...
                    break;
                case 'r':  /* TAA MOD 3/98: change to round to nearest even */
                {
                    LVAL cut;
                    xlsave1(cut);
                    if (plusp(y)) {
                        if (plusp(r)) {
                            cut = div2(y, FIXTWO);
                            if (gtr2(r, cut) || (oddp(n) && equ2(r, cut)))
                                { n = add1(n); r = sub2(r, y); }
                        }
                        else {
                            cut = div2(y, FIXNEGTWO);
                            if (lss2(r, cut) || (oddp(n) && equ2(r, cut)))
                                { n = sub1(n); r = add2(r, y); }
                        }
                    }
                    else {
                            ...
                            int, which)
{
    ...
    if (rd != 0.0) {
        switch (which) {
            ...
            {
                double dummy;
                int isOdd = MODF(fabs(nd)/2.0, &dummy) >= 0.5; 
                if (yd > 0.0) { 
                    if (rd > 0.0) { 
                        if (rd > 0.5 * yd || (isOdd && rd == 0.5 * yd)) { nd++; rd -= yd; }
                    }
                    else {
                        if (rd < -0.5 * yd || (isOdd && rd == -0.5 * yd)) { nd--; rd += yd; }
                    }
...
LVAL xexpt(V)
{
    ...
    /* rational or complex rational base, integer power */
    /* Code change suggested by Luke Tierney to better handle real base and
       integer power */
        ...
        if (fixp(base) && fixp(power)) {
            ...
            /* TAA 11/96, add 0.5 to fix rounding problem in GCC
            1/97 -- do floor to fix problem with negative values */
            if (infixnumrange(fv)) {
                if (fp > 0.0)
...
LVAL xasin(V)
{
  ...
  case FLONUM:
    {
      double dx = makefloat(x);
      if (dx <= 1.0 && dx >= -1.0) /* Fixed 3/12/96 */
