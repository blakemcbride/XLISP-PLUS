...
LOCAL LVAL NEAR entermsg _((LVAL cls, LVAL msg));
LOCAL LVAL NEAR sendmsg _((LVAL obj, LVAL cls, LVAL sym));
LOCAL int  NEAR getivcnt _((LVAL cls, int ivar));
LOCAL int  NEAR listlength _((LVAL list));
LOCAL VOID NEAR xputobj _((LVAL fptr, LVAL val));
LOCAL LVAL NEAR xlclass _((char *name, int vcnt));
LOCAL VOID NEAR xladdivar _((LVAL cls, char *var));
LOCAL VOID NEAR xladdmsg _((LVAL cls, char *msg, int offset));
LOCAL int NEAR getivcnt _((LVAL cls, int ivar));
LOCAL int NEAR listlength _((LVAL list));
...
LOCAL VOID NEAR xputobj(fptr,val)
  LVAL fptr; LVAL val;
{
    LVAL temp;
...
LOCAL LVAL NEAR xlclass(name,vcnt)
  char *name; int vcnt;
{
    LVAL sym,cls;
...
LOCAL VOID NEAR xladdivar(cls,var)
  LVAL cls; char *var;
{
...
LOCAL VOID NEAR xladdmsg(cls,msg,offset)
...
LOCAL int NEAR getivcnt(cls,ivar)
  LVAL cls; int ivar;
{
    LVAL cnt;
...
LOCAL int NEAR listlength(list)
  LVAL list;
{
    int len;
